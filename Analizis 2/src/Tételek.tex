...
\]
Ekkor az $a\in\D_f$ pontot az $f$ lokális minimumhelyének nevezzük, $f(a)$ pedig az $f$ lokális maximuma.
\paragraph{4.}
...
\textbf{Megjegyzés:} fogalmam nincs melyik jó lehet választani majd kiderül kis zh-n :)
\begin{enumerate}
\item $f(x)=x^3$ esetén $f'(x)=3x^2$ ami csak a $0$ pontban $0$ egyértelműen nem lokális szélsőérték $x<0$ esetén $x^3 < 0$ és $x>0$ esetén $x^3>0$
\item $f(x)=3$ (bármilyen konstans függvény) ugyanis $f'(x)=0$ ezáltal ez minden $a$-ra teljesül.
\end{enumerate}
\paragraph{6.}
